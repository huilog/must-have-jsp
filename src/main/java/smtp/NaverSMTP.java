...
public class NaverSMTP {
	//네이버 SMTP 서버 정보와 인증 정보는 변하지 않으므로 final 인스턴스 변수로 선언. 생성자에서 변수 초기화 진행.
	...
	public void emailSending(Map<String, String> mailInfo) 
		...
		//1. 세션 생성
		Session session = Session.getInstance(serverInfo, auth); //인스턴스 변수로 선언한 서버 정보와 인증 정보 사용
		session.setDebug(true);
		
		//2. 메시지 작성 (매개변수로 받은 map 컬렉션에서 메일 내용을 뽑아 메시지 작성)
