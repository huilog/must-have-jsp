...
//채팅창이 열리면 대화창, 메시지 입력창, 대화명 표시란으로 사용할 DOM 객체를 변수에 저장
...
function sendMessage() {
	//대화창에 표시
	chatWindow.innerHTML += "<div class='myMsg'>" + chatMessage.value + "</div>"; //대화창에 먼저 표시
	webSocket.send(chatId + '|' + chatMessage.value); //'대화명|메시지' 형태로 조립하여 서버로 전송
...
webSocket.onmessage = function(event) {
	...
	if (content != "") {
		if(content.match("/")) { //귓속말인지 '/' 포함 여부 확인
			if(content.match(("/" + chatId))) { //자신에게 보낸 메시지인지 확인
				var temp = content.replace(("/" + chatId), "[귓속말] : "); // 귓속말 명령어 부분을 '[귓속말] : ' 문자열로 치환하여 채팅창에 출력
